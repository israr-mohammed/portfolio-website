...
- [x] **S10: SEO and sharing**
  ...
  - Shipped: meta descriptions, canonicals, OG/Twitter tags (no images — see D36), JSON-LD (Person/WebSite on home; Article/BreadcrumbList on both case studies), `robots.txt`, `sitemap.xml`, `favicon.svg`/`favicon.ico`/`apple-touch-icon.png`, and `404.html` noindex. All five changed/new HTML pages pass the W3C Nu validator with zero messages, and all JSON-LD passes schema.org's structured-data validator with zero errors/warnings. See D33–D39.
...
| D33 | Home, resume and both case-study titles keep their already-shipped wording rather than the older illustrative title pattern in §6.4 | Owner confirmed in S10 that the shipped titles (matching the S1-approved headline, D11) are final; §6.4's example text predates that approval and isn't a literal requirement. |
| D34 | Sukoon's S10 meta description is deliberately generic: "A private personal/family organizer application built with Flask, PostgreSQL and JavaScript." | Owner correction, S10: earlier drafts mentioning the shared household organizer, tasks, shopping and trackers were rejected as too specific for a public, indexable description, in line with D29's household-privacy stance. |
| D35 | Canonical URLs, `sitemap.xml` and `robots.txt` all target the stable Cloudflare Pages URL `https://portfolio-website-bo0.pages.dev/`, with no custom domain | Consistent with D17 (custom domain deferred to S13) and §3.1's confirmed portfolio URL; avoids the "both pages.dev and custom domain indexed" risk flagged in §10. |
| D36 | No `og:image`/`twitter:image` shipped in S10; OG/Twitter tags are text-only (title, description, url, type, site_name, card) | Owner decision, S10: the 1200×630 typographic share images called for in §7 need real design work and no invented/stock art is allowed; rather than block S10, images are deferred to a separate follow-up once production is decided. `twitter:card` is `summary` (not `summary_large_image`) until an image exists. |
